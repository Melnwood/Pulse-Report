  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
      ...
      <div style={{ display:"inline-flex", border:"1px solid #E2D3C2", borderRadius:8, overflow:"hidden" }}>
        <button type="button" onClick={() => onChange(own)} style={btn(!shared, "#5A4A3B")}><IconLock size={13}/> {ownLabel}</button>
...
export function VisibilityChip({ visibility, onClick }) {
  ...
  return (
    <button onClick={onClick} title={`${visAudience(visibility)} — click to change`}
      ...
      {shared ? <IconEye size={11}/> : <IconLock size={11}/>}{visLabel(visibility)}<span style={{ fontSize:8, fontWeight:600, opacity:.65, marginLeft:2 }}>▾ change</span>
    </button>
  );
}
